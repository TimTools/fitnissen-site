---
layout: landing
type: article
title: Nieuwe website online!
title_html: "Persoonlijke training op maat"
tags: blogpost
full_width: true
header:
  theme: dark

article_header:
  type: overlay
  theme: dark
  align: center
  height: 100vh
  background_color: '#203028'
  background_image:
    gradient: 'linear-gradient(135deg, rgba(34, 139, 87 , .4), rgba(139, 34, 139, .4))'
    src: /assets/images/posts/2025-07-09-Nieuwe-website-online.png
excerpt: >
  Na een week flink zweten en hard werken is het eindelijk zover: mijn nieuwe website is online!

data:
  sections:
    - title: "Not for dreamers, but for those who put in the work!"
      height: 100vh
      theme: dark
      type: cover
      excerpt: |
        Na een week lang zweten — niet alleen in de gym, maar ook achter het scherm — is het zover: mijn nieuwe website is live! 💻🔥
        Met een frisse look, duidelijke structuur en inspirerende content wil ik je hier nog beter begeleiden op jouw weg naar resultaat.

        Of je nu net begint, vastloopt of toe bent aan een next level: deze site is jouw startpunt voor persoonlijke groei — fysiek én mentaal.

        👊 Geen loze beloftes. Geen shortcuts. Alleen échte inzet, op maat gemaakt voor jou.<br>
        Kijk rustig rond, lees de blogs, en als je klaar bent om serieus aan jezelf te werken… weet je me te vinden.

        Welkom. Let’s get to work!
---

# <!--more-->
# Hier komt de inhoud van de eerste echte post. Dit is een voorbeeld van hoe je een blogpost kunt schrijven in Jekyll met de TeXt-thema. Je kunt hier tekst, afbeeldingen, links en andere elementen toevoegen om je verhaal te vertellen.